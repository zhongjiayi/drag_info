    ...
    <template #main>
      <div class="layer-sortable-list">
        <ul class="currentLists">
          <li
            v-for="elem of showElemList"
            :key="elem.Name"
            class="currentList-list"
            :class="{ active: elem.pIndex === elemActive }"
            :draggable="filterKey === '' && !isEdit && !extend"
            @dragstart="dragstart(index)"
            @dragover="allowDrop"
            @drop="dropHandle(index)"
            @click.left.stop="changeActive(elem)"
            @dblclick.stop="startEdit(elem)"
          >
            <div class="screen-item">
              <svg class="svg-icon" aria-hidden="true">
                <path
                  v-for="item of componentsList[elem.elemType].sPath"
                  :d="item"
                ></path>
              </svg>
              <input
                v-if="isEdit && elem === elem.pIndex"
                type="text"
                v-model="editName"
                ref="elemInput"
                placeholder="请输入元素名称"
                @keyup.enter="endEdit(elem.index)"
                @blur="endEdit(elem)"
              />
            ...
            <div
              v-if="!isEdit"
              class="handleButton"
              @mouseenter="mouseenterHandle(elem)"
              @mouseleave="mouseleaveHandle(elem)"
            >
              <svg
                class="svg-icon icon-ellipsis"
                viewBox="0 0 18 18"
                aria-hidden="true"
              >
                <path
                  d="M16 11a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm-7 0a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm-7 0a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"
                ></path>
              </svg>
            </div>
          </li>
...
export default class PageList extends Vue {
  ...
  get showElemList() {
    return this.elemList.filter(
      (item) => item.elemName.indexOf(this.filterKey) > -1
    );
  }

  get componentsList() {
    return this.components.reduce(
      (o: { [x: string]: any }, cur: { type: string | number }) =>
        (o[cur.type] = cur) && o,
      {}
    );
  }
  ...
  endEdit(elem: Elem): void {
    if (this.editName !== "") {
      this.isEdit = false;
      const res =
        this.checkString(this.editName) ||
        (this.editName === elem.elemName
          ? ""
          : this.checkDupName(this.editName, this.programsList));
      if (res === "") {
  ...
  dropHandle(index: number) {
    if (this.dragIndex !== -1) {
      this.elemList.splice(
        index,
        0,
        this.programsList.splice(this.dragIndex, 1)[0]
      );
      this.dragIndex = -1;
    }
  }
